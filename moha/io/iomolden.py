...
class Molden(object):
    ...
    def write_GTO(self,handler):
        ...
        for i,index in enumerate(orbital_set_index):
            ...
            self.write_atom_orbital_header(handler,self.orbs.basis[index])
            for j in range(len(self.orbs.basis[index].exps)):
                handler.write("\t\t\t %f \t %f \n"%(self.orbs.basis[index].exps[j],self.orbs.basis[index].coefs[j]))

    def write_MO(self,handler):
        handler.write("[MO]\n")
        for i in range(self.wf.dim):
    ...
    def atom_index_filter(self):
        ...
        """
        list = []
        for i,basis in enumerate(self.orbs.basis):
    ...
    def write_MO_header(self,handler,i):
        occ_alpha = [1]*self.wf.occ['alpha']+[0]*(self.wf.dim-self.wf.occ['alpha'])
        occ_beta = [1]*self.wf.occ['beta']+[0]*(self.wf.dim-self.wf.occ['beta'])
        handler.write("Sym=%s\n"%('A'))
        handler.write("Ene=%f\n"%(self.wf.eorbitals[i]))
    ...
    def write_MO_coefficient(self,handler,i):
        for j in range(self.wf.dim):
            handler.write("{} \t {} \n".format(j+1,self.wf.coefficient[j,i]))
