...
class Molden(object):
    """
    """
    def __init__(self,filename,mol,orb,wfn):
        """
        """
    ...
    def write_GTO(self,handler):
        ...
        for i,index in enumerate(orbital_set_index):
            ...
            self.write_atom_orbital_header(handler,self.orb[index])
            for j in range(len(self.orb[index].exps)):
                handler.write("\t\t\t %f \t %f \n"%(self.orb[index].exps[j],self.orb[index].coefs[j]))
    ...
    def atom_index_filter(self):
        ...
        """
        list = []
        for i,basis in enumerate(self.orb):
